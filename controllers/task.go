...
import (
	...
	libcrons "github.com/mkhappy/gowebcron/crons"
	"github.com/mkhappy/gowebcron/jobs"
	"github.com/mkhappy/gowebcron/libs"
...
func (this *TaskController) Add() {
	...
	if this.isPost() {
		...
		task.GroupId, _ = this.GetInt("task_group")
		task.TaskName = strings.TrimSpace(this.GetString("task_name"))
		task.Description = strings.TrimSpace(this.GetString("task_description"))
		...
		task.Command = strings.TrimSpace(this.GetString("task_command"))
		task.Timeout, _ = this.GetInt("task_timeout")
		// mail
		task.Notify, _ = this.GetInt("task_notify")
		notifyEmail := strings.TrimSpace(this.GetString("task_notify_email"))
		if notifyEmail != "" {
		...
		if _, err := libcrons.Parse(task.CronSpec); err != nil {
